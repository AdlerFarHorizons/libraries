...
L3G4200D.h - Source file for the L3G4200H triple axis gyro / angular rate sensor Arduino library
...
L3G4200D::L3G4200D(bool bSDO) {
	m_Address = L3G4200D_Base_Address;
	if (bSDO)
		++m_Address;
}

uint8_t L3G4200D::Initialize(uint16_t rate) {
	// Defaults:
	// 100 Hz data rate
	// 12.5 bandwidth cutoff
	// Power down mode: normal
	// X, Y, Z axes enabled
	Write(L3G4200D_CTRL_REG1, 0b00001111);
	...
	Write(L3G4200D_CTRL_REG2, 0b00000000);
	...
	Write(L3G4200D_CTRL_REG3, 0b00000000);
	...
	Write(L3G4200D_CTRL_REG5, 0b00000000);	
	return SetRate(rate);
}

uint8_t L3G4200D::SetRate(uint16_t rate) {
	...
	switch ( rate ) {
		case 250:
			Write(L3G4200D_CTRL_REG4, 0b00000000);
			m_Scale = 0.00875;
			return 0;
			break;
		case 500:
			Write(L3G4200D_CTRL_REG4, 0b00010000);
			m_Scale = 0.0175;
			return 0;
			break;
		case 2000:
			Write(L3G4200D_CTRL_REG4, 0b00100000);
			m_Scale = 0.07;
			return 0;
			break;
		default:
			return L3G4200D_ErrorCode_1_Num;
	}
}

bool L3G4200D::CheckWhoAmI() {
	uint8_t* buffer = Read(L3G4200D_WHO_AM_I, 1);
	return (buffer[0] == 0b11010011);
}

GyroRaw L3G4200D::ReadRawAxis() {
  uint8_t* buffer = Read(L3G4200D_OUT_X_L | 0x80, 6);
  GyroRaw raw = GyroRaw();
...
GyroScaled L3G4200D::ReadScaledAxis() {
	GyroRaw raw = ReadRawAxis();
	return Raw2Scaled(raw);
}

GyroScaled L3G4200D::Raw2Scaled(GyroRaw raw) {
	GyroScaled scaled = GyroScaled();
...
char* L3G4200D::GetErrorText(uint8_t errorCode)
{
	if(L3G4200D_ErrorCode_1_Num == 1)
		return L3G4200D_ErrorCode_1;
	
	return "Error not defined.";
}
		
void L3G4200D::Write(uint8_t address, uint8_t data) {
...
uint8_t* L3G4200D::Read(uint8_t address, uint8_t length) {
	Wire.beginTransmission(m_Address);
	Wire.write(address);
	Wire.endTransmission();
