...
int HMC5883L::SetConfiguration(uint8_t averaging, uint8_t rate, uint8_t bias)
{
	Write(ConfigurationRegisterA, averaging | rate | bias);
	return 0;
}

uint8_t HMC5883L::GetStatus()
{
	uint8_t* buffer = Read(StatusRegister, 1);
	return buffer[0];
}
